//#region node_modules/.nitro/vite/services/ssr/assets/weeks-C2aOtEIw.js
var DAY_LABEL = [
	"Seg",
	"Ter",
	"Qua",
...
function buildWeek(n, month, phase, title, smart, capstoneSlice, days, extra) {
...
var weeks = [
	buildWeek(1, 1, 0, "Sistema de estudo", "Até domingo o Obsidian abre este vault, o GitHub existe, Node 22 e Git fazem um commit, e o caderno tem a primeira página.", "Nome provisório da Vitrine, 5 referências visuais salvas, 1 frase de proposta.", [
		{
			kind: "main",
	...
	buildWeek(2, 1, 1, "CSS que ainda falta", "Até domingo você reconstrói um layout de landing em Grid/Flex sem framework, responsivo em 390px e desktop.", "Esboço em papel da home: hero, grade de produtos, rodapé.", [
		{
			kind: "main",
	...
	buildWeek(3, 1, 1, "JS assíncrono e DOM", "Até domingo um carrossel ou filtro de produtos funciona em JS vanilla, com estado previsível.", "JSON local com 3 produtos (nome, imagem, url de afiliado).", [
		{
			kind: "main",
	...
	buildWeek(4, 1, 1, "TypeScript entra", "Até domingo o micro da landing está tipado, `tsc --noEmit` passa, e o mês 1 fecha no GitHub.", "Tipos `Product` e `AffiliateLink` no repo da Vitrine.", [
		{
			kind: "main",
	...
	buildWeek(5, 2, 2, "Next.js entra de verdade", "Até domingo um app Next (App Router + TS) roda, com layout, uma rota e os 3 produtos renderizados no servidor.", "Repo da Vitrine vira Next. A landing vanilla fica como arquivo histórico, não se mistura.", [
		{
			kind: "main",
	...
	buildWeek(6, 2, 2, "Figma → código", "Até domingo a home do Figma e a home do Next divergem no máximo em 2 decisões conscientes, documentadas.", "Espaçamento e type da home iguais ao frame 390.", [
		{
			kind: "design",
	...
	buildWeek(7, 2, 2, "CSS avançado + Tailwind fase 2", "Até domingo você usa Grid, container queries ou scroll-driven CSS em um bloco da home, e Tailwind não apagou seus tokens.", "Seção hero com composição mais densa (não só card repetido).", [
		{
			kind: "main",
	...
	buildWeek(8, 2, 2, "Capstone shell", "Até domingo `/`, `/p/[id]` e um JSON de produtos existem; deploy de preview no ar.", "Rotas públicas da vitrine, ainda sem admin e sem banco.", [
		{
			kind: "main",
	...
	buildWeek(9, 3, 2, "Dados no Next + Go começa", "Até domingo os produtos vêm de um módulo único; loading e erro existem; você fez a sessão 1 da Tour of Go.", "Fonte de verdade dos produtos ainda é arquivo TS/JSON, não banco.", [
		{
			kind: "main",
	...
	buildWeek(10, 3, 2, "Framer Motion", "Até domingo os cards entram com motion de 200–400ms, respeitam reduced-motion, e não usam lib nenhuma além de Motion.", "Motion só no que o usuário percebe: lista e CTA, não em tudo.", [
		{
			kind: "main",
	...
	buildWeek(11, 3, 2, "Formulário admin (falso)", "Até domingo existe `/admin` protegido só por um segredo local (ainda sem auth real) para editar o JSON da semana.", "Admin feio pode. Admin que funciona, melhor. Visual de admin não é a trilha de design.", [
		{
			kind: "main",
	...
	buildWeek(12, 3, 2, "Testes no front e Pin 1", "Até domingo 2 testes Vitest passam, 1 smoke Playwright da home passa, e o repo está pinado com README honesto.", "Vitrine front v1 pública. Este é o pin 1 do GitHub.", [
		{
			kind: "main",
	...
	buildWeek(13, 4, 2, "GSAP entra", "Até domingo uma seção da home tem ScrollTrigger (pin ou fade) estável em mobile.", "Uma seção, não o site inteiro em GSAP.", [
		{
			kind: "main",
			title: "GSAP core",
	...
	buildWeek(14, 4, 2, "Páginas extras da vitrine", "Até domingo existem `/` `/p/[id]` `/sobre` (quem é você, disclaimer de afiliado).", "Disclaimer legal simples: links de afiliado, você pode ganhar comissão.", [
		{
			kind: "main",
	...
	buildWeek(15, 4, 2, "Eletiva curta ou folga de profundidade", "Até domingo você ou fecha um buraco da rubrica do pin 1, ou faz UMA eletiva (a11y ou SEO OG). Não as duas.", "Capstone só recebe correção de bug, zero feature.", [
		{
			kind: "main",
	...
	buildWeek(16, 4, 2, "Recuperação 1", "Semana de folga estruturada: caderno, 2 sessões leves, hiperfoco opcional na trilha design. Sem tópico novo obrigatório.", "Nada no capstone, a menos que esteja quebrado em produção.", [
		{
			kind: "folga",
	...
	buildWeek(17, 5, 3, "HTTP cru", "Até domingo você explica request/response, headers, cookies, CORS e status, e sobe um servidor Node de 40 linhas.", "Contrato da API da Vitrine escrito: `GET /products`, `POST /clicks`.", [
		{
			kind: "main",
	...
	buildWeek(18, 5, 3, "Express mini, depois Fastify", "Até domingo o mini Express existe (legado) e o mesmo hello está em Fastify com schema.", "A API da Vitrine nasce em Fastify, não em Express.", [
		{
			kind: "main",
	...
	buildWeek(19, 5, 3, "Fastify de verdade", "Até domingo há rotas de products e clicks, logs, e um teste de rota.", "Click ainda não persiste — conta em memória e já devolve 302 para a loja.", [
		{
			kind: "main",
	...
	buildWeek(20, 5, 3, "Postgres entra (sem ORM)", "Até domingo um Postgres (Neon gratuito ou Docker) tem a tabela products; o Fastify lê dela.", "SQL cru. Nenhuma ORM. Neon se a RAM não aguentar Docker.", [
		{
			kind: "main",
	...
	buildWeek(21, 6, 3, "SQL de verdade", "Até domingo você escreve SELECT com JOIN, WHERE, ORDER, LIMIT e explica um EXPLAIN simples.", "Tabela clicks. Modelo: product 1—N clicks.", [
		{
			kind: "main",
	...
	buildWeek(22, 6, 3, "Migrations", "Até domingo o schema sobe de arquivo, não de memória. Um colega (você no futuro) recria o banco do zero.", "Ferramenta simples (node-pg-migrate, dbmate, ou SQL numerado). Sem Prisma.", [
		{
			kind: "main",
	...
	buildWeek(23, 6, 3, "CRUD SQL no Fastify", "Até domingo admin fala com o banco: criar, listar, editar, desativar produto (sem DELETE físico).", "soft-delete: `active boolean`.", [
		{
			kind: "main",
	...
	buildWeek(24, 6, 3, "MySQL comparativo + fechar dados", "Até domingo você leu o caso de índices/escala (Shopify/MySQL) e escreveu 1 página: o que mudaria se a Vitrine fosse MySQL.", "Nenhuma migração para MySQL. Só cérebro.", [
		{
			kind: "main",
	...
	buildWeek(25, 7, 3, "Auth de verdade", "Até domingo o admin exige login. Cookie httpOnly. Sem JWT no localStorage.", "Um usuário: você. Sem 'social login' neste mês.", [
		{
			kind: "main",
			title: "Sessão vs JWT",
	...
	buildWeek(26, 7, 3, "OWASP na API", "Até domingo você percorre um checklist curto e corrige o que a Vitrine tem de buraco óbvio.", "Headers de segurança, validação, limites.", [
		{
			kind: "main",
			title: "XSS",
	...
	buildWeek(27, 7, 3, "Upload de imagem", "Até domingo o produto tem imagem: upload autenticado ou URL da plataforma, com limite de tamanho.", "CDN cara não entra. Cloudinary free, S3-compat, ou storage do host. Máquina fraca: não processe 20MB local.", [
		{
			kind: "main",
	...
	buildWeek(28, 7, 3, "Pin 2 · API + admin", "Até domingo o backend está no ar (Railway/Fly/Render — o mais simples), o admin autentica, o click-out funciona.", "Este é o pin 2. Pode ser o mesmo repo (monorepo leve) ou `vitrine-api`.", [
		{
			kind: "main",
			title: "Subir a API",
	...
	buildWeek(29, 8, 4, "Pagamentos (lab, não a vitrine)", "Até domingo um repo separado cobra 1 produto de teste (Stripe test mode) e mostra um PIX copiável.", "A vitrine de afiliado NÃO cobra o visitante. Este lab é o pin-opcional de SaaS.", [
		{
			kind: "main",
	...
	buildWeek(30, 8, 4, "WebSocket", "Até domingo o admin vê cliques ao vivo (ou um feed ping). 1 conexão, 1 canal.", "Feature da Vitrine: tosta/linha nova quando alguém clica.", [
		{
			kind: "main",
			title: "WS vs HTTP",
	...
	buildWeek(31, 8, 4, "Pipeline de scroll-video", "Até domingo você tem um clip de 3–6s (gerado por IA se quiser) convertível em sequência e um proto no browser.", "Ainda não precisa estar na home. Lab na trilha design.", [
		{
			kind: "design",
	...
	buildWeek(32, 8, 4, "Capstone: stats + proto na home", "Até domingo o admin tem contagem por produto e o proto de scroll-video cabe no hero (ou em `/studio`).", "Se o hero ficar pesado, `/studio` é a decisão certa. Anote.", [
		{
			kind: "main",
	...
	buildWeek(33, 9, 4, "Scroll-video de produção", "Até domingo a peça de motion está no ar em URL própria ou `/studio`, 390px usável, fallback ok.", "Este trabalho vira o pin 3 se o resto do mês fechar.", [
		{
			kind: "design",
			title: "Direção",
	...
	buildWeek(34, 9, 4, "Three.js lab (contido)", "Até domingo uma cena mínima (1 objeto, 1 luz, 1 câmera) roda. Não entra na home se não servir o produto.", "Lab. Se hiperfoco, para no domingo.", [
		{
			kind: "main",
	...
	buildWeek(35, 9, 4, "Home cinematográfica", "Até domingo a home pública mistura hierarquia, motion e a peça de scroll sem parecer template.", "Refino. Proibido feature de backend nesta semana.", [
		{
			kind: "design",
	...
	buildWeek(36, 9, 4, "Pin 3 · assinatura + Vitrine v1", "Até domingo o GitHub tem o pin 3 (peça de motion ou a Vitrine inteira se a home carregar a assinatura) e um case study.", "Capstone v1: pública, com produtos reais, click-out, admin, stats.", [
		{
			kind: "main",
			title: "Case study",
	...
	buildWeek(37, 10, 5, "Recuperação 2 + mapa Nest", "Semana mais leve: você lê o mapa Nest e fecha buracos da v1. Sem reescrever a Vitrine em Nest.", "Nest é módulo de emprego, não o backend da Vitrine.", [
		{
			kind: "folga",
			title: "Bugs da v1",
	...
	buildWeek(38, 10, 5, "Nest: DTO, pipe, guard", "Até domingo o nest-lab tem CRUD in-memory, validação e um guard de API key.", "Profundidade: o suficiente para uma prova prática júnior/pleno.", [
		{
			kind: "main",
	...
	buildWeek(39, 10, 5, "Nest + SQL (opcional raso) e TypeORM/Prisma de olho", "Até domingo você conecta o nest-lab no Postgres OU documenta por que ficou in-memory (corte de profundidade).", "ORM aparece como leitura. O capstone continua SQL cru.", [
		{
			kind: "main",
	...
	buildWeek(40, 10, 5, "Go REST + SQL (sprint)", "Até domingo o catálogo em Go lê e cria product no mesmo banco (ou num schema `go_lab`).", "Evidência Go: API pequena, SQL cru, testes. Não é segundo capstone.", [
		{
			kind: "go",
	...
	buildWeek(41, 11, 5, "PWA", "Até domingo a Vitrine instala no Android (ícone, offline raso da home).", "Mesmo código web. Sem store.", [
		{
			kind: "main",
	...
	buildWeek(42, 11, 5, "Capacitor no Android", "Até domingo um APK ou build de debug abre a Vitrine no seu Android. Sem Play Store.", "Não instale Android Studio no dia 1 se a RAM morrer: use o fluxo mais leve da doc, ou um só wrap no final de semana de hiperfoco.", [
		{
			kind: "main",
	...
	buildWeek(43, 11, 5, "Go concorrência aplicada", "Até domingo o go-catalog tem um endpoint que dispara N trabalhos com limite de goroutines (ex.: checar URLs).", "Útil e pequeno. Sem crawler-monstro.", [
		{
			kind: "go",
	...
	buildWeek(44, 11, 5, "Fechar mobile + Go", "Até domingo PWA, APK, go-catalog e nest-lab têm README. A Vitrine v1.1 só se for bugfix.", "Mês 12 é renda, não feature.", [
		{
			kind: "main",
	...
	buildWeek(45, 12, 6, "Portfólio como produto", "Até domingo um site (pode ser a própria Vitrine `/sobre` + GitHub) conta a história do ano em 3 peças.", "Não comece um portfólio-framework novo. Use Next que você já domina.", [
		{
			kind: "main",
			title: "Narrativa",
	...
	buildWeek(46, 12, 6, "Oferta freelance", "Até domingo existe uma oferta de 1 parágrafo para landing com motion / vitrine de afiliados / site de 1 produto.", "Renda primeiro caminho: B depois de C. Vaga é a semana seguinte.", [
		{
			kind: "main",
			title: "Oferta",
	...
	buildWeek(47, 12, 6, "Vagas BR + remoto", "Até domingo 10 candidaturas saíram (BR e/ou remoto) com CV e GitHub alinhados ao que você de fato fez.", "Não se candidate a sênior Go. Pleno Next/Node ou júnior forte.", [
		{
			kind: "main",
	...
	buildWeek(48, 12, 6, "Fechamento do vault", "Até domingo o changelog pessoal está escrito, a fase 2 priorizada, e o que não coube foi cortado por escrito.", "v1 do plano congela. Você não reescreve o vault por ansiedade.", [
		{
			kind: "caderno",
	...
	buildWeek(49, 12, 6, "Recuperação 3 · folga de fim de linha", "Semana de ar. Só se candidatura/freelance estiver quente: responde. Sem tópico novo.", "Capstone só se produção quebrar.", [
		{
			kind: "folga",
			title: "Descanso",
	...
	buildWeek(50, 12, 6, "Hiperfoco residual 1", "Se a renda ainda não veio: 7h desta semana em candidatura + 1 melhoria visível no portfólio. Sem curso novo.", "Melhoria = copy ou performance da peça, não 'migrar para Nest'.", [
		{
			kind: "main",
			title: "Mais 5 vagas",
			outcome: "Mandou."
	...
	buildWeek(51, 12, 6, "Hiperfoco residual 2", "Igual à 50, com teto. Se um processo avançou, o plano da semana vira 'passar na entrevista' — SQL, HTTP, Next.", "Estudar o que a vaga pede, não o que o YouTube pede.", [
		{
			kind: "main",
	...
	buildWeek(52, 12, 6, "Fim do v1", "Você fecha o ano: vault v1 congelado, evidências listadas, fase 2 com data de início ou não.", "Se a renda veio, o plano cumpriu. Se não, o plano ainda cumpriu: há produto, há prova, há linha para o ano 2.", [
		{
			kind: "caderno",
...
//#endregion
export { weeks as i, phases as n, sessionKindLabel as r, DAY_LABEL as t };
